	       ...
	       struct daos_llink **link)
{
	...
	if (arg->pca_map != NULL) {
		rc = pl_map_update(pool->sp_uuid, arg->pca_map, false);
		if (rc != 0) {
...
pool_iter_cb(daos_handle_t ph, uuid_t co_uuid, void *data)
{
	...
	case DAOS_INTENT_REBUILD: {
		...
		rc = dtx_resync(ph, arg->po_uuid, co_uuid, arg->version);
		if (rc != 0) {
	...
	default:
		return ds_cont_iter(ph, arg->po_uuid, co_uuid, cont_iter_cb,
				    data, VOS_ITER_OBJ);
	}
}
 ...
 **/
int
ds_pool_iter(uuid_t pool_uuid, rebuild_iter_cb_t callback, void *data,
